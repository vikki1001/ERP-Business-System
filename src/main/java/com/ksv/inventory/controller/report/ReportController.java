package com.ksv.inventory.controller.report;
...
import com.ksv.inventory.interfaces.ReportInterface;
...
public class ReportController implements Initializable, ReportInterface {
    ...
    @FXML
    private TableColumn<Invoice, Long> idColumn,phoneColumn;
    @FXML
    private TableColumn<Invoice, String> employeeColumn,nameColumn,emailColumn,dateColumn; // nameColumn,emailColumn
    @FXML
    private TableColumn<Invoice, Double> totalColumn, vatColumn, discountColumn, // dueColumn
            payableColumn,dueColumn,paidColumn, returnedColumn;
    @FXML
    private TextField searchField,IdField,EmployeeField,NameField,PhoneField,
    EmailField,TotalField,VatField,DiscountField,PayableField,DueField,PaidField,
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        /*name , mobile, Email Id fetch and to show the table*/
    ...
    private void loadData(){
        ...
        for(Invoice item : model.getInvoices()) {
        	totalDueAmount = totalDueAmount + item.getDue();
        	totalPayable = totalPayable + item.getPayable();
        	totalPaid = totalPaid + item.getPaid();
        	totalDiscount = totalDiscount + item.getDiscount();
        	totalVat = totalVat + item.getVat();
        	totalReturn = totalReturn + item.getReturned();
        	totalReturn = totalReturn + item.getReturned();
        	totAmount = totAmount + item.getTotal();
        } // Double.parseDouble(priceField.getText());
        SetData();
    }
    
    private void SetData() {
    	IdField.setText("Grand Total");
    	EmployeeField.setText("-");
    	NameField.setText("-");
    	PhoneField.setText("-");
    	EmailField.setText("-");
    	DueField.setText(String.valueOf(df.format(totalDueAmount)));
    	PayableField.setText(String.valueOf(df.format(totalPayable)));
    	PaidField.setText(String.valueOf(df.format(totalPaid)));
    	DiscountField.setText(String.valueOf(df.format(totalDiscount)));
    	VatField.setText(String.valueOf(df.format(totalVat)));
    	ReturnField.setText(String.valueOf(df.format(totalReturn)));
    	TotalField.setText(String.valueOf(df.format(totAmount)));
    	DateField.setText("-");
    	
    	/*
    	 searchField,IdField,EmployeeField,NameField,PhoneField,
    EmailField,TotalField,VatField,DiscountField,PayableField,DueField,PaidField,
    ReturnField,DateField; 
